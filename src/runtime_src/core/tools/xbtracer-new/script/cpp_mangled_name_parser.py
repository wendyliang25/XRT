...
def gen_wrapper_funcs(funcs: set, class_dict: dict, out_cpp_dir: str):
    ...
    func_info_list = sort_funcs(funcs)
    class_file_map = dict()
    class_file_map['xrt::aie::device'] = "hook_xrt_aie.cpp"
    class_file_map['xrt::aie::program'] = "hook_xrt_aie.cpp"
    ...
    class_file_map['xrt::ip::interrupt'] = "hook_xrt_ip.cpp"
    ...
    class_file_map['xrt::profile::user_event'] = "hook_xrt_profile.cpp"
    class_file_map['xrt::profile::user_range'] = "hook_xrt_profile.cpp"
    ...
    class_file_map['xrt::xclbin::arg'] = "hook_xrt_xclbin.cpp"
    class_file_map['xrt::xclbin::ip'] = "hook_xrt_xclbin.cpp"
    class_file_map['xrt::xclbin::kernel'] = "hook_xrt_xclbin.cpp"
    class_file_map['xrt::xclbin::mem'] = "hook_xrt_xclbin.cpp"
    ...
    for decl in funcs:
        ...
        func_c_match = re.search(r"([\w:]+)::([\w=*&-+<>\s]+)$", func_n)
        func_f = None
        if func_c_match:
            func_c = func_c_match.group(1)
            func_n = func_c_match.group(2)
        else:
            sys.exit(f"function\'{decl}\' doesn't have class information.")
        if func_c_match:
            func_f = class_file_map[func_c_match.group(1)]
        if not func_f:
            func_f = func_file_map[func_s]
        if not func_f:
            sys.exit(f"failed to get generated cpp file for \'{func_n}\'.")
...
"""
  ...
  arg->set_pointer_fieldi(std::string(reinterpret_cast<const char*>(&this_pimpl_ptr), sizeof(this_pimpl_ptr)));
"""
                    out.write(f"{lines}")
            lines = f"""
